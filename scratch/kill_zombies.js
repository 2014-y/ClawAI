...
try {
    console.log('Scanning for leftover ClawAI / openclaw Node.js processes safely...');
    ...
    if (output) {
        ...
        for (const line of lines) {
            ...
            // 只要路径里包含了 ClawAI、openclaw、或者 sandbox 相关的 node，且不是我们当前的运行进程 PID
            const lowerPath = path.toLowerCase();
            const currentPid = process.pid.toString();
            
            if (pid && pid !== currentPid && (lowerPath.includes('clawai') || lowerPath.includes('openclaw') || lowerPath.includes('node-sandbox'))) {
    ...
    // 强杀所有 ClawAI.exe 残留进程 (防止客户端主进程僵尸挂起)
    console.log('Scanning for ClawAI.exe zombie processes...');
    try {
        const tasklist = execSync('tasklist', { encoding: 'utf8' }).toLowerCase();
        if (tasklist.includes('clawai.exe')) {
            execSync('taskkill /f /im ClawAI.exe');
            console.log('Successfully killed all ClawAI.exe instances.');
        }
    } catch(err) {
        console.warn('No active ClawAI.exe or failed to kill:', err.message);
    }
