...
export default function createPlugin(runtime) {
  ...
  try {
    ...
    // [suppressed] [${pluginName}] ⚠️  配置读取异常，使用默认值: ${e.message}
    pluginConfig = {};
  }
  ...
  try {
    ...
    // [suppressed] [${pluginName}] ❌ 无法创建数据目录: ${e.message}
    // 降级到默认目录
  ...
  function atomicAppend(filePath, line) {
    try {
      ...
      // [suppressed] [${pluginName}] ❌ 文件写入失败: ${e.message}
      // 清理临时文件
  ...
  function analyzeTeacherVoice(teacherAnswer) {
    ...
    try {
      ...
      // [suppressed] [${pluginName}] ⚠️  口吻分析异常: ${e.message}
      // 分析失败返回默认值，不中断主流程
    }
...
要求回答专业、清晰、有条理。`;
  ...
  function saveTrainingData(record) {
    try {
      ...
      // [suppressed] [${pluginName}] ❌ 保存训练数据失败: ${e.message}
      stats.totalErrors++;
    }
  }
  ...
  async function teachAndLearn(question, deliveryContext) {
    ...
    if (teacherResult.teacherAnswer) {
      studentLearnInBackground(question, teacherResult.teacherAnswer, deliveryContext)
        .catch(err => {
          stats.totalErrors++;
          // [suppressed] [${pluginName}] ❌ [后台] 学生学习失败: ${err?.message || String(err)}
        });
    }
  ...
  return {
    ...
    async onAfterResponse(context) {
      ...
      Promise.resolve(teachAndLearn(question, deliveryContext))
        ...
        .catch(error => {
          stats.totalErrors++;
          // [suppressed] [${pluginName}] ❌ Teach-Learn 流程异常: ${error?.message || String(error)}
        });
    },
...
// [suppressed] dual-model-trainer summary failure
    }
  },
    async onShutdown() {
      // 计算延迟统计
